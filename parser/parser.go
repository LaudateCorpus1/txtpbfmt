...
type Config struct {
	...
	// AddFieldSortOrder() but 1+ fields under that node in the textproto aren't specified in the field
	// order. This won't fail for nodes that don't have a field order specified at all. Use this to
	// strictly enforce that your field order config always orders ALL the fields, and you're willing
	// for new fields in the textproto to break parsing in order to enforce it.
...
func sortAndFilterNodes(parentNodeName string, nodes []*ast.Node, c Config) error {
	...
	if c.RequireFieldSortOrderToMatchAllFieldsInNode && len(fieldOrderSorter.unknownFields) > 0 {
		return fmt.Errorf("field order config is missing the following unrecognized parent.child fields: %v", fieldOrderSorter.unknownFields)
	}
	return nil
}
...
type sortableByFieldOrder struct {
	...
	// are not in the config, the "parent.child" field names will be appended to this list.
...
func (s *sortableByFieldOrder) Less(i, j int) bool {
	...
	if !s.nodes[i].IsCommentOnly() {
		...
		lprio = order[name]
		if lprio == 0 {
			s.unknownFields = append(s.unknownFields, fmt.Sprintf("%s.%s", s.parentNodeName, name))
		}
	}
